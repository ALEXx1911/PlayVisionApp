...
class TestCompetitionAPI:
    def test_competition_success(self, api_client, competition_factory):
        competition_factory(title="La Liga", slug="la-liga")
        url = reverse('competition-details', kwargs={'ctitle': 'la-liga'})
        response = api_client.get(url)

        assert response.status_code == status.HTTP_200_OK
        assert set(response.data.keys()) >= {'competition', 'top_scorers', 'top_media_players','most_yellow_cards','top_goalkeepers'}

    def test_competition_include_top_scorers(self,competition_setup, api_client, player_factory, player_competition_stats_factory):
    ...
    def test_competition_include_top_media_players(self, api_client,competition_setup, player_factory, player_competition_stats_factory):
        competition, season = competition_setup()
    ...
    def test_competition_include_most_yellow_cards(self, api_client, competition_setup, player_factory, player_competition_stats_factory):
        competition, season = competition_setup()
    ...
    def test_competition_include_top_goalkeepers(self, api_client, competition_setup, player_factory, player_competition_stats_factory):
        competition, season = competition_setup()
...
@pytest.fixture
def competition_setup(competition_factory, season_factory):
